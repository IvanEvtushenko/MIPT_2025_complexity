...
\usepackage[dvipsnames]{xcolor} % даёт расширенные имена цветов (RoyalBlue, OliveGreen, BrickRed и т.д.)
\usepackage{listings}           % окружение lstlisting для вставки кода

\definecolor{codebg}{rgb}{0.97,0.97,0.97} % светлый фон для кода

\lstdefinestyle{py}{
  language=Python,                  % включаем правила языка Python
  basicstyle=\ttfamily\small,       % шрифт кода
  keywordstyle=\color{RoyalBlue}\bfseries, % ключевые слова (def, for, return...) — синие + жирные
  commentstyle=\color{OliveGreen}\itshape, % комментарии — зелёные + курсив
  stringstyle=\color{BrickRed},     % строки — красные
  numbers=left,                     % номера строк слева
  numberstyle=\tiny\color{gray},    % номера строк серые
  stepnumber=1,                     % нумеровать каждую строку
  numbersep=8pt,                    % отступ номеров от кода
  frame=single,                     % рамка
  rulecolor=\color{black},          % цвет рамки
  backgroundcolor=\color{codebg},   % фон блока кода
  breaklines=true,                  % перенос длинных строк
  columns=fullflexible,             % аккуратная моноширинная верстка
  keepspaces=true,                  % сохранять пробелы
  showstringspaces=false,           % не показывать пробелы в строках
  tabsize=4,                        % tab = 4 пробела
  xleftmargin=12pt,                 % сдвиг блока вправо
  aboveskip=8pt, belowskip=8pt      % отступы сверху/снизу
}
...
\newtheorem{decision}{Решение}[section]
